{%- macro clickhouse__type_string() -%} {{ "Nullable(String)" }} {%- endmacro %}

{%- macro clickhouse__type_int() -%} {{ "Nullable(Int32)" }} {%- endmacro %}

{%- macro clickhouse__type_float() -%} {{ "Nullable(Float32)" }} {%- endmacro %}

{%- macro clickhouse__type_boolean() -%} {{ "Nullable(Bool)" }} {%- endmacro %}
...
{% macro clickhouse__split_part(string_text, delimiter_text, part_number) -%}
    splitByString(
        assumeNotNull({{ delimiter_text }}), assumeNotNull({{ string_text }})
    )[{{ part_number }}]
{%- endmacro %}
...
{% macro clickhouse__listagg(measure, delimiter_text, order_by_clause, limit_num) -%}
    {% if order_by_clause and " by " in order_by_clause -%}
        ...
        {% set arr = "arrayMap(x -> x.1, arrayReverseSort(x -> x.2, arrayZip(array_agg({}), array_agg({}))))".format(
            arr, order_by_field
        ) %}
    {% else -%} {% set arr = "array_agg({})".format(measure) %}
    {%- endif %}
